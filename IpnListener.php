...
class IpnListener
{
    ...
    protected function curlPost($encoded_data){
        ...
        $this->post_uri = $uri;
//file_put_contents('ipn.log', "...ABOUT TO START CURL..." . PHP_EOL, LOCK_EX | FILE_APPEND); //TESTING DELETE LATER
        $ch = curl_init();
...
//file_put_contents('ipn.log', "...PROCESSING CURL..." . PHP_EOL, LOCK_EX | FILE_APPEND); //TESTING DELETE LATER
        ...
        if ($this->response === false || $this->response_status == '0') {
            ...
            file_put_contents('ipn.log', "....CURL ERROR...".$errno.$errstr . PHP_EOL, LOCK_EX | FILE_APPEND); //TESTING DELETE LATER
        }
        
//file_put_contents('ipn.log', "...return curl response..." . PHP_EOL, LOCK_EX | FILE_APPEND); //TESTING DELETE LATER
    ...
    {
//file_put_contents('ipn.log', "START OF PROCESSIPN" . PHP_EOL, LOCK_EX | FILE_APPEND);//TESTING DELETE LATER
        try
        {
            $this->requirePostMethod();		// processIpn() should check itself if data is POST
//file_put_contents('ipn.log', "IS A POST, BEGIN THE TRY BLOCK" . PHP_EOL, LOCK_EX | FILE_APPEND);//TESTING DELETE LATER
            // Read POST data
            ...
            //TESTING
//file_put_contents('ipn.log', "finished reading post, about to use curl" . PHP_EOL, LOCK_EX | FILE_APPEND);
...
//file_put_contents('ipn.log', "verified!" . PHP_EOL, LOCK_EX | FILE_APPEND);//TESTING DELETE LATER
                return true;
    ...
    {
//file_put_contents('ipn.log', "START OF REQUIRE POST METHOD" . PHP_EOL, LOCK_EX | FILE_APPEND); //TESTING DELETE LATER
